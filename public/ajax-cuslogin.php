<?php  
 ...
 if($_POST){
     ...
     $itm_id = mysqli_real_escape_string($conn, $_POST['pat']);
     ...
     if($login_count > 0) {
         ...
         $upd = mysqli_query($conn, "UPDATE user SET itm_id = '$itm_id' WHERE email = '$email'");
                
                $get_nam = mysqli_query($conn, "SELECT * FROM user WHERE email = '$email'");
